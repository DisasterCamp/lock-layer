...
public class LockManager {
                ...
                while (iterator.hasNext()) {
                    ...
                    if ((value.getExpireCount().intValue() >= LockConfigUtil.getMaxExpireCount() * getAnInt(value))
                        ...
                        if (shutdown) {
                            ...
                            LoggerUtil.println(Thread.currentThread().getClass(), String.format("key = %s,If the number of consecutive times is exceeded, the lock is released", next.getKey()));
                            iterator.remove();
                            handlerLockHeartRemovedProcessor(value);
                        } else {
                            LoggerUtil.println(Thread.currentThread().getClass(), String.format("ScheduledFuture = %s,isn't cancel", next.getValue().getFuture()));
    ...
    public static LockManager create() {
        return create(LockConfig.build());
    }
    ...
    public static LockManager create(LockConfig lockConfig) {
        ...
        Runtime.getRuntime().addShutdownHook(new ShutDownHook(lockManager));
        return lockManager;
    }
